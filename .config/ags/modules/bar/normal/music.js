...
const BarResource = (name, icon, command, circprogClassName = 'bar-batt-circprog', textClassName = 'txt-onSurfaceVariant', iconClassName = 'bar-batt', poll_time=4000) => {
    ...
    const widget = Button({
        ...
        child: Box({
            ...
            setup: (self) => self
                .poll(poll_time, () => execAsync(['bash', '-c', command])
                    .then((output) => {
            ...
            child: Box({
                children: [
                    BarResource('RAM Usage', 'memory', `LANG=C free | awk '/^Mem/ {printf("%.2f\\n", ($3/$2) * 100)}'`,
                        'bar-ram-circprog', 'bar-ram-txt', 'bar-ram-icon', 5000),
                    Revealer({
                        revealChild: true,
                        ...
                        child: Box({
                            ...
                            children: [
                                BarResource('Swap Usage', 'swap_horiz', `LANG=C free | awk '/^Swap/ {if ($2 > 0) printf("%.2f\\n", ($3/$2) * 100); else print "0";}'`,
                                    'bar-swap-circprog', 'bar-swap-txt', 'bar-swap-icon', 5000),
                                BarResource('CPU Usage', 'settings_motion_mode', `${App.configDir}/scripts/cpu_proc.sh 0.5`,
                                    'bar-cpu-circprog', 'bar-cpu-txt', 'bar-cpu-icon', 2000),
                            ]
                        }),
                        // setup: (self) => self.hook(Mpris, label => {
                        //     const mpris = Mpris.getPlayer('');
