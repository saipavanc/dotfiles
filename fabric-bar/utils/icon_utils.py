...
def icon_image_from_name(icon_name: str, size: int = 16):
    image = Gtk.Image()
...
def update_image_from_name(image: Gtk.Image, icon_name: str, size: int = 16):
    ...
    image
        GTK Image
    icon_name
        Name of the icon identifiable by gtk theme. Image object will be emptied if None.
    size, optional
    ...
    if icon_name is None:
        image.clear()
        return image
    # check if icon exists in the theme
    try:
